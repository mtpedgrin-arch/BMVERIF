...
/** Lista todos los productos del proveedor (por categoría para evitar error 500) */
export async function supplierGetProducts(category = "facebook") {
  // El endpoint /products sin parámetros devuelve 500; con ?category funciona
  const url = `${BASE}/products${category ? `?category=${encodeURIComponent(category)}` : ""}`;
...
export async function supplierCreateOrder(productId, qty) {
  ...
  try {
    ...
    // El proveedor devolvió HTML u otra respuesta no-JSON (ej. 402 saldo, 502 caída, WAF block)
    const rawText = await res.text().catch(() => "");
    data = { message: `Respuesta inválida HTTP ${res.status}${rawText ? ` — ${rawText.slice(0, 120)}` : ""}` };
  }
