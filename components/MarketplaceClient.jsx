...
// ─── GLOBAL ORDERS DB ─────────────────────────────────────────────────────────
const ORDERS_DB = [
  { id: "ORD-001", userId: 1, userEmail: "charlyxentorix@gmail.com", userName: "Charly Xentorix", items: [{ name: "BM Facebook · Verified · Europe/USA", price: 33.00, qty: 1 }], subtotal: 33.00, discount: 0, coupon: null, total: 33.00, network: "TRC20", txHash: "", status: "paid", date: "2026-02-09" },
  { id: "ORD-002", userId: 1, userEmail: "charlyxentorix@gmail.com", userName: "Charly Xentorix", items: [{ name: "BM Facebook · Ukraine · Reinstated", price: 34.50, qty: 1 }], subtotal: 34.50, discount: 0, coupon: null, total: 34.50, network: "BEP20", txHash: "", status: "pending", date: "2026-02-20" },
  { id: "ORD-003", userId: 2, userEmail: "john@example.com", userName: "John Doe", items: [{ name: "BM Facebook · Brazil · $50 Limit", price: 27.00, qty: 2 }], subtotal: 54.00, discount: 5.40, coupon: "DEMO10", total: 48.60, network: "TRC20", txHash: "abc123", status: "pending", date: "2026-02-25" },
];
...
const MOCK_USERS = [
  { id: 1, email: "charlyxentorix@gmail.com", name: "Charly Xentorix", password: "user123", balance: 12.11, joined: "2025-11-01" },
  { id: 2, email: "john@example.com", name: "John Doe", password: "john123", balance: 5.50, joined: "2026-01-15" },
];
const ADMIN = { email: "admin@shop.com", password: "admin2026" };
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @keyframes pulseGreen { 0%,100%{box-shadow:0 0 0 0 rgba(26,138,74,0.3)} 50%{box-shadow:0 0 0 6px rgba(26,138,74,0)} }
  ...
  .auth-tabs { display: flex; border: 1.5px solid var(--border); border-radius: 11px; overflow: hidden; margin-bottom: 18px; }
  .auth-tab { flex: 1; padding: 9px; font-size: 13px; font-weight: 600; border: none; background: none; color: var(--muted); transition: all 0.15s; }
  ...
  /* ORDER CONFIRM ROW */
  .confirm-order-row { display: flex; align-items: center; gap: 10px; }
...
const AuthModal = ({ onSuccess, onClose }) => {
  ...
  const submit = () => {
    ...
    if (tab === "login") {
      ...
      if (user) onSuccess(user); else setError("Email o contraseña incorrectos.");
    } else {
      ...
      const nu = { id: Date.now(), email: form.email, name: form.name, password: form.password, balance: 0, joined: today() };
      MOCK_USERS.push(nu); onSuccess(nu);
    }
  };
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-sub">Inicia sesión o crea una cuenta para completar tu compra</div>
        ...
        {tab === "register" && <div className="form-group"><label className="form-label">Nombre</label><input className="form-input" placeholder="Tu nombre" value={form.name} onChange={set("name")} /></div>}
        <div className="form-group"><label className="form-label">Email</label><input className="form-input" type="email" placeholder="email@ejemplo.com" value={form.email} onChange={set("email")} /></div>
        <div className="form-group"><label className="form-label">Contraseña</label><input className="form-input" type="password" placeholder="••••••••" value={form.password} onChange={set("password")} onKeyDown={e => e.key === "Enter" && submit()} /></div>
        {tab === "register" && <div className="form-group"><label className="form-label">Confirmar</label><input className="form-input" type="password" placeholder="••••••••" value={form.confirm} onChange={set("confirm")} /></div>}
        <button className="btn btn-primary btn-full" onClick={submit}>{tab === "login" ? "→ Entrar y pagar" : "✓ Crear cuenta y pagar"}</button>
        <div style={{ marginTop: 12, background: "var(--bg)", borderRadius: 9, padding: "8px 12px", fontSize: 12, color: "var(--muted)" }}>
          Demo: <strong>charlyxentorix@gmail.com</strong> / <strong>user123</strong>
        </div>
      </div>
    </div>
  );
...
const PaymentModal = ({ cart, user, coupon, finalTotal, onClose, onSuccess }) => {
  const [network, setNetwork] = useState(null); // "TRC20" | "BEP20"
  const [txHash, setTxHash] = useState("");
  const [copied, setCopied] = useState(false);
  const [step, setStep] = useState(1); // 1=select network, 2=show address
          ...
          <>
            ...
            <div className="wallet-box">
              <div className="wallet-box-label">
                {WALLETS[network].logo} Wallet USDT {network} — {WALLETS[network].network}
              </div>
              <div className="wallet-address">{WALLETS[network].addr}</div>
              <button className="wallet-copy-btn" onClick={copy}>
            ...
            <div className="tx-input-wrap">
              ...
              <input
                className="tx-input"
                placeholder="Ej: 0xabc123def456... o TXhash..."
                value={txHash}
                onChange={e => setTxHash(e.target.value)}
              />
              <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 4 }}>
                Pegar el hash acelera la confirmación por parte del admin.
              </div>
            </div>

            <div style={{ display: "flex", gap: 8 }}>
              <button className="btn btn-usdt" style={{ flex: 1, justifyContent: "center" }} onClick={() => onSuccess(network, txHash)}>
                ✓ Ya realicé el pago
              </button>
        ...
        {cart.length > 0 && (
          <>
            ...
            <div className="cart-footer">
              ...
              <button className="checkout-btn" onClick={() => onCheckout(appliedCoupon, total)}>
                ₮ Pagar con USDT →
              </button>
            </div>
          </>
...
const UserAccount = ({ user, userOrders, liked, onToggleLike, onGoShop }) => {
  ...
  const myOrders = userOrders.filter(o => o.userId === user.id);
  const favProducts = PRODUCTS.filter(p => liked[p.id]);
  const changePw = () => {
    if (!pwData.current || !pwData.newpw || !pwData.confirm) return setPwMsg({ t: "e", msg: "Completa todos los campos." });
    if (pwData.newpw !== pwData.confirm) return setPwMsg({ t: "e", msg: "Las contraseñas no coinciden." });
    setPwMsg({ t: "s", msg: "✓ Contraseña actualizada." });
  ...
  return (
    <div className="page">
      <div style={{ display: "flex", alignItems: "center", gap: 13, marginBottom: 20 }}>
        ...
        <div><div style={{ fontFamily: "Syne", fontSize: 19, fontWeight: 800 }}>Hola, {user.name.split(" ")[0]} 👋</div><div style={{ fontSize: 13, color: "var(--muted)" }}>{user.email}</div></div>
      </div>
          ...
          {favProducts.length === 0 ? (
            ...
            <>
              ...
              <div style={{ display: "flex", flexDirection: "column", background: "var(--surface)", border: "1.5px solid var(--border)", borderRadius: 12, overflow: "hidden", boxShadow: "var(--shadow)" }}>
                {favProducts.map(p => (
                  <div key={p.id} style={{ display: "flex", alignItems: "center", padding: "14px 18px", borderBottom: "1px solid var(--border)", gap: 14, transition: "background 0.12s" }}
                    ...
                    <div style={{ width: 52, height: 52, borderRadius: 10, background: "linear-gradient(145deg,#1877F2,#0d5bbf)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flexShrink: 0, gap: 2 }}>
                      ...
                      <span style={{ fontSize: 8, fontWeight: 700, color: "#fff", letterSpacing: 0.3 }}>Facebook</span>
                    </div>
                    {/* info */}
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 3, lineHeight: 1.3 }}>{p.name}</div>
                      <div style={{ fontSize: 12, color: "var(--muted)", lineHeight: 1.4 }}>{p.details}</div>
                      <div style={{ display: "flex", gap: 8, marginTop: 6, flexWrap: "wrap" }}>
                        <span className="chip chip-stock">In Stock: <strong>{p.stock} pcs.</strong></span>
                        <span className="chip chip-sales">Sales: <strong>{p.sales}</strong></span>
                      </div>
                    </div>
                    {/* price + actions */}
      ...
      {tab === "settings" && (
        <div className="settings-grid">
          <div className="card">
            ...
            <div className="form-group"><label className="form-label">Nombre</label><input className="form-input" defaultValue={user.name} /></div>
          ...
          <div className="card">
            ...
            {pwMsg && <div className={pwMsg.t === "e" ? "error-msg" : "success-msg"}>{pwMsg.msg}</div>}
            {[["current", "Contraseña actual"], ["newpw", "Nueva contraseña"], ["confirm", "Confirmar nueva"]].map(([k, label]) => (
              <div className="form-group" key={k}><label className="form-label">{label}</label><input className="form-input" type="password" value={pwData[k]} onChange={e => setPwData(p => ({ ...p, [k]: e.target.value }))} /></div>
            ))}
            <button className="btn btn-primary btn-sm" onClick={changePw}>Actualizar</button>
          </div>
        </div>
      )}
    </div>
              ...
              {orders.slice().reverse().map(o => (
                <tr key={o.id} style={{ background: o.status === "pending" ? "#FFFBEB" : "transparent" }}>
                  ...
                  <td>
                    ...
                    <div style={{ fontSize: 11, color: "var(--muted)" }}>{o.userEmail}</div>
                  </td>
                  <td style={{ fontSize: 12, maxWidth: 180 }}>{o.items.map(i => `${i.name.slice(0, 25)}...`).join(", ")}</td>
                  <td><span className="tag-network">{o.network}</span></td>
                  <td><strong style={{ color: "var(--usdt)" }}>{fmtUSDT(o.total)}</strong>
                    {o.coupon && <div style={{ fontSize: 10, color: "var(--purple)" }}>🏷 {o.coupon}</div>}
                  </td>
                  <td>
                    {o.txHash
                      ? <code style={{ fontSize: 10, color: "var(--blue)" }}>{o.txHash.slice(0, 14)}...</code>
                  ...
                  <td>
                    {o.status === "pending" ? (
                      <button className="confirm-btn" onClick={() => onConfirm(o.id)}>
                        ✓ Confirmar pago
                      </button>
                    ) : (
      ...
      <div className="admin-content">
        {section === "overview" && (
          <>
            ...
            <div className="stats-row">
              <div className="stat-card"><div className="stat-label">Usuarios</div><div className="stat-value" style={{ color: "var(--blue)" }}>{MOCK_USERS.length}</div></div>
            ...
            {pendingCount > 0 && (
              <div style={{ background: "var(--amber-light)", border: "1px solid var(--amber-border)", borderRadius: 12, padding: "12px 16px", marginBottom: 18, fontSize: 13, color: "var(--amber)", display: "flex", alignItems: "center", gap: 8 }}>
                ⏳ <strong>{pendingCount} orden{pendingCount > 1 ? "es" : ""} esperando confirmación de pago</strong>
                  ...
                  <tbody>{MOCK_USERS.map(u => (
                    <tr key={u.id}>
                      ...
                      <td><div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <div style={{ width: 27, height: 27, background: "var(--red-light)", color: "var(--red)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 11 }}>{u.name[0]}</div>
                        {u.name}</div></td>
...
export default function App() {
  ...
  const handleAuthSuccess = u => { setUser(u); setShowAuth(false); setShowPayment(true); };
  ...
  const loginAdmin = () => {
    if (adminCreds.email === ADMIN.email && adminCreds.password === ADMIN.password) { setIsAdmin(true); setShowAdminLogin(false); setAdminError(""); }
    else setAdminError("Credenciales incorrectas.");
  };
    ...
    return (
      <div className="app">
        ...
        <div className="topbar">
          <div className="logo">NPPR<span>SHOP</span><span style={{ fontSize: 11, background: "var(--red)", color: "#fff", padding: "2px 8px", borderRadius: 6, marginLeft: 7 }}>ADMIN</span></div>
          <div style={{ fontSize: 13, color: "var(--muted)" }}>Panel de administración</div>
          <button className="btn btn-outline btn-sm" onClick={() => setIsAdmin(false)}>← Volver a la tienda</button>
        </div>
  ...
  return (
    <div className="app">
      <style>{css}</style>
      <div className="topbar">
        <div className="logo" onClick={() => setView("shop")}>NPPR<span>SHOP</span></div>
        <div className="topbar-right">
          {user ? (
            <>
              ...
              <button className="btn btn-outline btn-sm" onClick={() => { setUser(null); setView("shop"); }}>Salir</button>
            </>
          ) : (
            <>
              <button className="btn btn-outline btn-sm" onClick={() => setShowAuth(true)}>Iniciar sesión</button>
              <button style={{ background: "none", border: "none", fontSize: 12, color: "var(--muted)", padding: "6px 8px" }} onClick={() => setShowAdminLogin(true)}>Admin</button>
            </>
          )}
      ...
      {showAdminLogin && (
        <div className="modal-overlay" onClick={() => setShowAdminLogin(false)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 340 }}>
            <div style={{ fontSize: 32, marginBottom: 10 }}>🔐</div>
            <div className="modal-title">Panel Admin</div>
            <div className="modal-sub">Acceso restringido</div>
            {adminError && <div className="error-msg">{adminError}</div>}
            <div className="form-group"><label className="form-label">Email</label><input className="form-input" value={adminCreds.email} onChange={e => setAdminCreds(p => ({ ...p, email: e.target.value }))} placeholder="admin@shop.com" /></div>
            <div className="form-group"><label className="form-label">Contraseña</label><input className="form-input" type="password" value={adminCreds.password} onChange={e => setAdminCreds(p => ({ ...p, password: e.target.value }))} onKeyDown={e => e.key === "Enter" && loginAdmin()} /></div>
            <button className="btn btn-primary btn-full" onClick={loginAdmin}>Entrar →</button>
            <div style={{ marginTop: 12, fontSize: 12, color: "var(--muted)", background: "var(--bg)", borderRadius: 8, padding: "8px 12px" }}>
              Demo: admin@shop.com / admin2026
            </div>
          </div>
        </div>
      )}
